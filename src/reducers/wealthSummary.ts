...
export const wealthSummaryReducer = (state = wealthSummaryDefaulState, action: any): WealthSummaryReducer => {
    switch (action.type) {
        case WealthSummaryActions.SET_WEALTH_SUMMARIES:
            return { wealthSummaries: action.wealthSummaries, wealthSummaryId: action.wealthSummaries[0]?.id };
